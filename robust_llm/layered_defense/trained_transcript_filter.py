...
@ComponentRegistry.register("TrainedTranscriptFilter")
class TrainedTranscriptFilter(OutputComponent):
    def __init__(
        self,
    ...
    def __call__(
        ...
        message_lists = [
            message_list.filter(MessageFilter.TRANSCRIPT)
            for message_list in data.message_lists
        ]
        texts = [
            message_list.format_transcript(rng=self.rng)
            for i, message_list in enumerate(message_lists)
        ]
        inputs = self.filter.tokenize(texts, padding_side="right", return_tensors="pt")
