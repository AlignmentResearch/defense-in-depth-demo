...
@ComponentRegistry.register("WildGuardInputFilter")
class WildGuardInputFilter(InputComponent):
    def __init__(
        self,
        ...
        """Initialize a WildGuard input filter."""
        ...
        assert self.filter.generation_config is not None
        self.filter.generation_config.pad_token_id = self.tokenizer.pad_token_id
    ...
    def __call__(
        ...
        prompts = [message_list[-1].content for message_list in data.message_lists]
        responses = ["" for _ in range(len(message_lists))]
        assert isinstance(self.filter, GenerationMixin)
        scores = wildguard_scores(self.filter, self.tokenizer, prompts, responses)

        # Get probability of 'yes' for harmful prompt.
        prompt_harm_probs = scores[:, 0, 0].detach().cpu()

        results = []
        for i in range(len(message_lists)):
            result = ComponentResult(
                # Output is 0 for benign, 1 for jailbreak.
                flag=prompt_harm_probs[i].item() > self.threshold,
                name="WildGuardInputFilter",
                score=prompt_harm_probs[i],
            )
